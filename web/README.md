...
如果之前使用 `--sugar_local_dump` 生成了 pkl 文件，可以离线上传：

```bash
# 方式二：从已有的 pkl 文件上传
python -m pressure.pressure \
    --upload_web_from_pkl /path/to/result.pkl ~/llm-pressure-web
```

**注意**: `--upload_web_from_pkl` 参数需要两个值：
1. pkl 文件路径（由 `--sugar_local_dump` 生成）
2. Git 仓库路径
...
| `--upload_web_from_pkl` | 从 pkl 文件上传，需指定两个值：pkl路径 和 仓库路径 | `result.pkl ~/llm-pressure-web` |
...
| 场景 | 参数组合 |
|------|---------|
| 压测时直接上传 | `--upload_web ~/llm-pressure-web` |
| 已有 pkl 文件，离线上传 | `--upload_web_from_pkl result.pkl ~/llm-pressure-web` |
| 先 dump pkl，后续再上传 | 先用 `--sugar_local_dump result.pkl`，后续用 `--upload_web_from_pkl` |
